Datastore C++ client [![Linux](https://github.com/hanzo-ds/cpp/actions/workflows/linux.yml/badge.svg)](https://github.com/hanzo-ds/cpp/actions/workflows/linux.yml) [![macOS](https://github.com/hanzo-ds/cpp/actions/workflows/macos.yml/badge.svg)](https://github.com/hanzo-ds/cpp/actions/workflows/macos.yml) [![Windows MSVC](https://github.com/hanzo-ds/cpp/actions/workflows/windows_msvc.yml/badge.svg)](https://github.com/hanzo-ds/cpp/actions/workflows/windows_msvc.yml) [![Windows mingw](https://github.com/hanzo-ds/cpp/actions/workflows/windows_mingw.yml/badge.svg)](https://github.com/hanzo-ds/cpp/actions/workflows/windows_mingw.yml)
=====

C++ client for [Hanzo Datastore](https://docs.hanzo.ai/datastore).
...
If you want the library to produce ColumnBool, set the CMake variable `DS_MAP_BOOL_TO_UINT8=OFF`.
...
- https://github.com/hanzo-ds/cpp/blob/master/.github/workflows/linux.yml
- https://github.com/hanzo-ds/cpp/blob/master/.github/workflows/windows_msvc.yml
- https://github.com/hanzo-ds/cpp/blob/master/.github/workflows/windows_mingw.yml
- https://github.com/hanzo-ds/cpp/blob/master/.github/workflows/macos.yml


## Example application build with datastore-cpp

There are various ways to integrate datastore-cpp with the build system of an application. Below example uses the simple approach based on
submodules presented in https://www.youtube.com/watch?v=ED-WUk440qc .

- `mkdir datastore-app && cd datastore-app && git init`
- `git submodule add https://github.com/hanzo-ds/cpp.git contribs/datastore-cpp`
...
int main()
{
    ...
    /// See https://docs.hanzo.ai/datastore/engines/table-engines/special/external-data
    {
        Block block1, block2;
        auto id = std::make_shared<ColumnUInt64>();
        id->Append(1);
...
add_subdirectory(contribs/datastore-cpp)

add_executable(${PROJECT_NAME} "app.cpp")

target_include_directories(${PROJECT_NAME} PRIVATE contribs/datastore-cpp/ contribs/datastore-cpp/contrib/absl)

target_link_libraries(${PROJECT_NAME} PRIVATE datastore-cpp-lib)
```
...
## Retries
If you wish to implement some retry logic atop of `datastore::Client` there are few simple rules to make you life easier:
- If previous attempt threw an exception, then make sure to call `datastore::Client::ResetConnection()` before the next try.
- For `datastore::Client::Insert()` you can reuse a block from previous try, no need to rebuild it from scratch.

See https://github.com/hanzo-ds/cpp/issues/184 for details.

## Asynchronous inserts
See https://docs.hanzo.ai/datastore/best-practices/asynchronous-inserts for details.

⚠ The asynchronous setting is different according to the datastore server version. The under example with datastore server version 24.8.4.13. ⚠

> Our strong recommendation is to use async_insert=1,wait_for_async_insert=1 if using asynchronous inserts. Using wait_for_async_insert=0 is very risky because your INSERT client may not be aware if there are errors, and also can cause potential overload if your client continues to write quickly in a situation where the Datastore server needs to slow down the writes and create some backpressure in order to ensure reliability of the service.

- Only use the SDK, do not need to change the datastore server config. Asynchronous inserts only work if the data is sent as SQL text format. Here is the example.
```cpp
// You can specify the asynchronous insert settings by using the SETTINGS clause of insert queries
datastore::Query query("INSERT INTO default.test SETTINGS async_insert=1,wait_for_async_insert=1,async_insert_busy_timeout_ms=5000,async_insert_use_adaptive_busy_timeout=0,async_insert_max_data_size=104857600 VALUES(10,10)");
client.Execute(query);

// Or by SetSetting
datastore::Query query("INSERT INTO default.test VALUES(10,10)");
query.SetSetting("async_insert", datastore::QuerySettingsField{ "1", 1 });
query.SetSetting("wait_for_async_insert", datastore::QuerySettingsField{ "1", 1 }); // strong recommendation
query.SetSetting("async_insert_busy_timeout_ms", datastore::QuerySettingsField{ "5000", 1 });
query.SetSetting("async_insert_max_data_size", datastore::QuerySettingsField{ "104857600", 1 });
query.SetSetting("async_insert_use_adaptive_busy_timeout", datastore::QuerySettingsField{ "0", 1 });
client.Execute(query);
...
```
- Change the datastore server users.xml, enable asynchronous inserts (available for the native data format). Here is the example.
```xml
<profiles>
        <!-- Default settings. -->
        <default>
