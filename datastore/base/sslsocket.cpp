...
//              << std::endl;
    ...
    throw datastore::OpenSSLError(prefix + std::to_string(error) + " : " + reason_str);
}

void configureSSL(const datastore::SSLParams::ConfigurationType & configuration, SSL * ssl, SSL_CTX * context = nullptr) {
#ifdef USE_BORINGSSL
    ...
    // OpenSSL (e.g. --@datastore_cpp//:tls=openssl with Bazel).
    (void)ssl;
    (void)context;
    if (!configuration.empty()) {
        throw datastore::OpenSSLError(
    ...
    for (const auto & kv : configuration) {
        ...
        if (err == 2 || (err == 1 && !value_present))
            ...
            throw datastore::OpenSSLError("Failed to configure OpenSSL: command '" + kv.first + "' needs no value");
        else if (err == -2)
            throw datastore::OpenSSLError("Failed to configure OpenSSL: unknown command '" + kv.first + "'");
        else if (err == -3)
            throw datastore::OpenSSLError("Failed to configure OpenSSL: command '" + kv.first + "' requires a value");
        else
            throw datastore::OpenSSLError("Failed to configure OpenSSL: command '" + kv.first + "' unknown error: " + std::to_string(err));
    }
#endif
}
...
SSL_CTX * prepareSSLContext(const datastore::SSLParams & context_params) {
    ...
    if (!ctx)
        throw datastore::OpenSSLError("Failed to initialize SSL context");
...
auto convertConfiguration(const decltype(datastore::ClientOptions::SSLOptions::configuration) & configuration)
{
    auto result = decltype(datastore::SSLParams::configuration){};
...
datastore::SSLParams GetSSLParams(const datastore::ClientOptions& opts) {
    const auto& ssl_options = *opts.ssl_options;
    return datastore::SSLParams{
    ...
    , ssl_(SSL_new(context.getContext()), &SSL_free)
{
    auto ssl = ssl_.get();
    if (!ssl)
        throw datastore::OpenSSLError("Failed to create SSL instance");
    ...
    if (const auto verify_result = SSL_get_verify_result(ssl); !ssl_params.skip_verification && verify_result != X509_V_OK) {
        ...
        throw datastore::OpenSSLError("Failed to verify SSL connection, X509_v error: "
