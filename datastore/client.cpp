...
#define CLIENT_NAME "datastore-cpp"
...
void Client::Impl::SelectWithExternalData(Query query, const ExternalTables& external_tables) {
    if (server_info_.revision < DBMS_MIN_REVISION_WITH_TEMPORARY_TABLES) {
       throw UnimplementedError("This version of Datastore server doesn't support temporary tables");
    }
...
bool Client::Impl::ReceiveException(bool rethrow, ServerError * error) {
    ...
    bool has_nested = false; // obsolete: https://github.com/hanzoai/datastore/blob/ef11941cf5a/src/IO/ReadHelpers.cpp#L2017
...
void Client::Impl::SendQuery(const Query& query, bool finalize) {
    ...
    if (server_info_.revision >= DBMS_MIN_REVISION_WITH_CLIENT_INFO) {
        ...
        info.client_version_major = DATASTORE_CPP_VERSION_MAJOR;
        info.client_version_minor = DATASTORE_CPP_VERSION_MINOR;
        info.client_version_patch = DATASTORE_CPP_VERSION_PATCH;
...
void Client::Impl::WriteBlock(const Block& block, OutputStream& output) {
    ...
    for (Block::Iterator bi(block); bi.IsValid(); bi.Next()) {
        ...
        // ref https://github.com/hanzoai/datastore/blob/39b37a3240f74f4871c8c1679910e065af6bea19/src/Formats/NativeWriter.cpp#L163
...
bool Client::Impl::SendHello() {
    ...
    WireFormat::WriteUInt64(*output_, DATASTORE_CPP_VERSION_MAJOR);
    WireFormat::WriteUInt64(*output_, DATASTORE_CPP_VERSION_MINOR);
