 ...
 * Works only when Datastore outputs JSON as strings and requires the setting
 ...
 *
 * Datastore does not accept empty strings as JSON; it requires an empty object ({}).
 * For nullable columns, each row marked a NULL must contain {}.
 * For convenience `datastore::ColumnNullableT<ColumnJSON>` automatically inserts {} for NULL rows.
 */
